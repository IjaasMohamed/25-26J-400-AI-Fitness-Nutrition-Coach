...
class _DetectionScreenState extends State<DetectionScreen> with SingleTickerProviderStateMixin {
  ...
  void _processGlobalRiskMetrics(Pose pose) {
    if (widget.exerciseDataModel.type == ExcerciseType.Squats || widget.exerciseDataModel.type == ExcerciseType.JumpingJack) {
    ...
    } else if (widget.exerciseDataModel.type == ExcerciseType.PushUps || widget.exerciseDataModel.type == ExcerciseType.PlankToDownwardDog) {
  ...
  void _checkScheduledTarget() {
    ...
    if (currentCount == targetReps && !_scheduleTriggered) {
      ...
      if (isLastSet) {
        // ✅ ALL sets done → voice + Show Heart Rate Dialog
        _tts.speak('Your $exerciseName sets are complete. Well done! Please enter your heart rate.');
        
        _showHeartRateDialog(context).then((heartRate) {
          _finishSet(heartRate: heartRate).then((_) {
            if (!mounted) return;
            if (widget.remainingSchedule.isEmpty) {
              Navigator.of(context).popUntil((route) => route.isFirst);
            } else {
              _launchNextScheduleItem();
            }
          });
        });
      } else {
              ...
              builder: (_) => RestTimerScreen(
                ...
                onRestComplete: () async {
                  ...
                  if (lastSetEndTime != null) {
                    ...
                    // Update last set with actual rest time
  ...
  Future<void> _finishSet({int? heartRate}) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Set $currentSetNumber completed & saved!'),
