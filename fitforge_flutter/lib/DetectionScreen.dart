...
class _MyHomePageState extends State<Detectionscreen> with SingleTickerProviderStateMixin {
  ...
  void detectHighKnees(Map<PoseLandmarkType, PoseLandmark> landmarks) {
    ...
    // Y-axis comparison (smaller Y = higher on screen)
    ...
    // Optional angle check for better accuracy
    double leftKneeAngle = calculateAngle(leftHip, leftKnee, leftAnkle);
    double rightKneeAngle = calculateAngle(rightHip, rightKnee, rightAnkle);

    bool isLeftKneeHigh =
        leftKneeY < leftHipY && leftKneeAngle < 120;

    bool isRightKneeHigh =
        rightKneeY < rightHipY && rightKneeAngle < 120;
...
// Pose Painter - draws pose landmarks on the camera preview
